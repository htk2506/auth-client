import { createApi, fetchBaseQuery } from '@reduxjs/toolkit/query/react';
import { getSessionToken, hasUnexpiredSessionToken } from './session-token-management';
import { CreateUserRequestBody, CreateUserResponseBody, LoginUserRequestBody, LoginUserResponseBody, UpdateUserRequestBody, UpdateUserResponseBody, User } from './types';

/**
 * Generates a value for an Authorization header using a bearer token.
 * @returns A string to use as Authorization header value.
 */
const generateAuthorizationHeader = () => {
    // Make sure there's an unexpired session token.
    if (!hasUnexpiredSessionToken()) {
        throw new Error("No unexpired session token found.");
    }

    // Retrieve session token from local storage
    const sessionToken = getSessionToken();

    // Return the header
    return `Bearer ${sessionToken}`;
}

// Define a service using a base URL and expected endpoints
export const apiSlice = createApi({
    reducerPath: 'api',
    baseQuery: fetchBaseQuery({ baseUrl: process.env.NEXT_PUBLIC_API_SERVER_URL }),
    tagTypes: ['CurrentUser'],
    endpoints: (builder) => ({
        getCurrentUser: builder.query<User, void>({
            query: () => ({
                url: `v1/users/me`,
                headers: {
                    ['Authorization']: generateAuthorizationHeader(),
                },
            }),
            providesTags: ['CurrentUser'],
        }),
        postLoginRequest: builder.mutation<LoginUserResponseBody, LoginUserRequestBody>({
            query: (loginRequest: LoginUserRequestBody) => ({
                url: `v1/sessions/login`,
                method: 'POST',
                body: loginRequest,
            }),
            invalidatesTags: ['CurrentUser'],
        }),
        postLogoutRequest: builder.mutation<LoginUserResponseBody, void>({
            query: () => ({
                url: `v1/sessions/logout`,
                method: 'POST',
                headers: {
                    ['Authorization']: generateAuthorizationHeader(),
                },
            }),
            invalidatesTags: ['CurrentUser'],
        }),
        putUpdateUserRequest: builder.mutation<UpdateUserResponseBody, UpdateUserRequestBody>({
            query: (updateUserRequest: UpdateUserRequestBody) => ({
                url: `v1/users/me`,
                method: 'PUT',
                headers: {
                    ['Authorization']: generateAuthorizationHeader(),
                },
                body: updateUserRequest,
            }),
            invalidatesTags: ['CurrentUser'],
        }),
        postCreateUserRequest: builder.mutation<CreateUserResponseBody, CreateUserRequestBody>({
            query: (createUserRequest: CreateUserRequestBody) => ({
                url: `v1/users`,
                method: 'POST',
                body: createUserRequest,
            }),
        }),
    }),
})

// Export hooks for usage in functional components
export const {
    useGetCurrentUserQuery,
    usePostLoginRequestMutation,
    usePostLogoutRequestMutation,
    usePutUpdateUserRequestMutation,
    usePostCreateUserRequestMutation,
} = apiSlice